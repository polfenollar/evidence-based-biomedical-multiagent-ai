...
Registers IngestionWorkflow, parse_and_validate_activity, and
write_to_lake_activity on the ``ingestion`` task queue.
...
async def _run_worker(shutdown_event: asyncio.Event) -> None:
    ...
    async with Worker(
        ...
        activities=[parse_and_validate_activity, write_to_lake_activity],
    ) as worker:
        logger.info(
